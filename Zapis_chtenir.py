# Процедура получения числа и добавление в списки последовательностей(двоек, троек и т.д.)
# (предположительно будет нужно для предсказания), перед этим файлы со словарями
# проверяются на пустоту, если пустой то с начала, если не пустой идет продолжение цепочки
# то есть здесь отрабатывается только запись, чтения здесь нет, есть только процедура восстановления
# цепочек последовательностей для этих целей используются буферные файлы вида "2_xoda.txt"...'6_xoda.txt'
# Сами последовательности запоминаются в файлах вида 2sh.txt...'7sh.txt'
import random
import json
...
lst_of6 = []
lst_of7 = []
# очищаем файл "100_xodov.txt" от прошлых записей
file_obj = open('100_xodov.txt', 'w')
file_obj.write('')
file_obj.close()
# вновь открываем уже пустой файл
